...
import { AGENCY_TYPE, CRITERION_FULFILLMENT_TYPE, DOCUMENT_CATEGORIES, DOCUMENT_TYPE_MAP, RuleSets, STAKEHOLDER_GROUP_ROLES } from '../src/lib/constants';
    ...
    await prisma.$transaction(async (prisma) => {
        ...
        await prisma.stakeholderGroup.createMany({
            data: [{
                ...
                Description: 'The client is the main user of My File. They create applications to different agencies and manage files for subsequent use on applications.',
            }, {
                Name: STAKEHOLDER_GROUP_ROLES.TRUSTED_USER,
                Description: 'A trusted user is someone who is trusted by the client to access their files and communicate with their case manager on their behalf',
            }, {
        ...
        const stakeholderGroupRoles: { Name: keyof typeof rolesMap | null; Description: string | null; StakeholderGroupId?: string }[] = stakeHolderGroup.map((ele) => {
    ...
    await prisma.$transaction(async (prisma) => {
        ...
        await prisma.workflowStageCriterion.createMany({
...

            {
                ...
                CaseWorkflowStageId: ckecklist1?.id,
                CriterionSubGroupName: 'For any family members who are not U.S citizens and have been detained and released or on parole',
        ...
        const HpdStage1 = await prisma.workflowStage.findFirst({ where: { StagePosition: 1, WorkflowId: HDPWorkflow?.id } });
        const HpdStage2 = await prisma.workflowStage.findFirst({ where: { StagePosition: 2, WorkflowId: HDPWorkflow?.id } });
