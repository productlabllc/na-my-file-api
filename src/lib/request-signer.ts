...
import { zonedTimeToUtc, utcToZonedTime, format as dateFormat } from 'date-fns-tz';
import { getIntegrationType, IntegrationType } from './oauthIntegration';

const getSigningKey = () => requireConfiguration(EnvironmentVariable.AUTH_SIGNING_KEY);
...
export const getNycIdSignature = (props: GetNycIdSignatureProps): string => {
  ...
  const stringToSign = `${method}${path}${getSortedQueryArgs(queryArgs)}${authorizationHeader}`;
  return hmacSha256(stringToSign, signingKey);
};

const getDateTimeArg = () => {
  return dateFormat(
    utcToZonedTime(zonedTimeToUtc(new Date(), Intl.DateTimeFormat().resolvedOptions().timeZone), 'America/New_York'),
    'MM/dd/yyyy HH:mm',
  );
};

export const signRequest = (method: 'GET' | 'POST', urlToSign: string, headers: Record<string, string>): string => {
