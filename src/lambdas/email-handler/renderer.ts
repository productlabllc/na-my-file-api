...
export const writeSnapshotFile = (name: string, content: string) => {
  if (WRITE_FILES) {
...
const loadTemplate = (templateName: string) => {
  if (!loadedTemplates[templateName]) {
    const templateResolvedPath = join(__dirname, templatePath, `${templateName}.mustache`);
...
export const renderTemplate = (template: string, data: any) => {
  return mustache.render(loadTemplate(template), data, partials);
};
